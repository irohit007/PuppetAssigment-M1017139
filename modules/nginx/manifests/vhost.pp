...
define apply( $domain="UNSET",$root="UNSET",$dir="UNSET"){  
 ...
 define addStaticFiles( $default_parent_root, $vhost_root , $vhost_domain , $vhost_dir ){
              ...
              file { $dir_tree :
                ...
                file {  ["$vhost_root/index.html"]:
                   owner   => 'ubuntu',
                          group   => 'ubuntu',
                          source => "puppet:///modules/nginx/$vhost_dir/index.html", # index.html was dropped under nginx/files/
                  mode    => '755',
                }
         file{  ["$vhost_root/systeminformation.html"]:
                  owner => 'ubuntu',
                  source => "puppet:///modules/nginx/$vhost_dir/systeminformation.html",
                  mode => '755',
}
   }
}
